...
import { hashSmashPattern, verifySmashPattern } from '../utils/smash';
...
interface LoginRequest {
  email: string;
  password?: string;
  smashPattern?: number[]; // Array of touch points for SMASH
}

interface SmashSetupRequest {
  pattern: number[]; // Array of touch points
}

interface SmashVerificationRequest {
  pattern: number[];
}
...
const generateToken = (userId: string): string => {
  return jwt.sign(
    { userId },
    process.env.JWT_SECRET || 'your_super_secure_jwt_secret_key_change_me',
    { expiresIn: '7d' }
  );
};
...
router.post('/login', async (req: Request, res: Response) => {
  try {
    const { email, password, smashPattern }: LoginRequest = req.body;
    ...
    if (smashPattern && user.smashPatterns.length > 0) {
      ...
      const smashValid = verifySmashPattern(smashPattern, smashRecord.patternHash, smashRecord.salt);
 ...
 */
router.post('/smash/setup', async (req: AuthRequest, res: Response) => {
  try {
    const { pattern }: SmashSetupRequest = req.body;
    ...
    const patternHash = hashSmashPattern(pattern, salt);
 ...
 */
router.post('/smash/verify', async (req: AuthRequest, res: Response) => {
  try {
    const { pattern }: SmashVerificationRequest = req.body;
    ...
    // Verify pattern
    const isValid = verifySmashPattern(pattern, smashRecord.patternHash, smashRecord.salt);
